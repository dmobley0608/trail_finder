...
exports.addTrails = async (req, res)=>{
   try {    
    ...
    let trail = await Trails.findOne({where: {name:{[Op.like]:`${req.body.name}`},length:req.body.length, ParkId:`${req.params.id}`}})
    ...
    trail = await Trails.create({...req.body, ParkId:req.params.id,  UserId:req.user.id})  
